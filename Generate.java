...
import java.io.*;
import java.util.*;
import static labyrinth.LabyrinthNew.field;

public class Generate extends LabyrinthNew {
    ...
    static void markIO(int x, int y, char c) {
        // Makes sure there is no obstacles in front on the entrance or exit
        field[x][y] = c;
        if (x == 0) {
            field[1][y] = free;
        } else if (x == height - 1) {
    ...
    static long init() throws Exception {
        ...
        // Create Character
        me = new Character(points[0][0], points[0][1]);
        field[me.currentPos[0]][me.currentPos[1]] = 'A';
        ...
        while (idx < points.length) {
            ...
            if (idx < nrIn + nrOut + nrKeys) {
                field[points[idx][0]][points[idx][1]] = key;
            } else if (idx < nrIn + nrOut + nrKeys + nrStaticTraps) {
                field[points[idx][0]][points[idx][1]] = StaticTrap;
            } else if (idx < nrIn + nrOut + nrKeys + nrStaticTraps + nrDynamicTraps) {

                //create Dynamic Trap objects
                DynamicTrap trap = new DynamicTrap(points[idx][0], points[idx][1], me);

                listofTraps.add(trap); // just in case we need to access them later, i.e stopping the timers

                field[points[idx][0]][points[idx][1]] = DynamicTrap;
            } else {
    ...
    static void connect(int x1, int y1, int x2, int y2) {
        ...
        } else if (choice == 2 || field[x1][y1] == StaticTrap || field[x1][y1] == DynamicTrap || field[x1][y1] == free) {
            dirx(minY, minX, maxX);
            dirx(maxY, minX, maxX);
            diry(minX, minY, maxY);
            diry(maxX, minY, maxY);
    ...
    static long read() {
        try {
              FileInputStream fileInput = new FileInputStream(new File("parameters.txt"));
    ...
    static void write(long seed) {
        ...
        try {
            try (FileOutputStream outFile = new FileOutputStream(new File(propFileName))) {
